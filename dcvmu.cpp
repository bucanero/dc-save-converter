...
char* VirtualFile::getPath() {
	return(path);
}

void VirtualFile::setPath(char *p) {
	strcpy(path, p);
}
...
uint8* DCIFile::swapData(uint8 *raw) {
	...
	for (ssize_t i=0; i<getDCFile()->getSize(); i+=4) {
		tmp[3]=raw[i];
		tmp[2]=raw[i+1];
		tmp[1]=raw[i+2];
		tmp[0]=raw[i+3];
...
int VMIFile::readData() {
	...
	fs_close(f);
	loadHeader(&header);
	sprintf(tmp, "%s%s.VMS", getPath(), getResourceName());
...
int RAWFile::readData() {
	...
	df->setName(cutName(getFileName()));
	fseek(f, 0, SEEK_END);
	df->setSize(ftell(f));
	fseek(f, 0, SEEK_SET);
	raw=new uint8[df->getSize()];
...
int RAWFile::writeData() {
	return(0);
}

char* RAWFile::cutName(char *fn) {
	int r, i=-1;
